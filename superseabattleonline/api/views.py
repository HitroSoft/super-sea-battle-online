...
class Game(object):
    ...
    def add_first_player(self, data):
        ...
        self.first_player_to_message_queue.append(
            {"name": "waiting-for-rival", "id": self.first_player_response_id, "start": int(time.time())})
    ...
    def attach_ai_player(self):
        ...
        self.first_player_response_id += 1
        self.first_player_to_message_queue.append(
            {"name": "game-started-move-on", "id": self.first_player_response_id, "start": int(time.time())})
        self.second_player_response_id += 1
        self.second_player_to_message_queue.append(
            {"name": "game-started-move-off", "id": self.second_player_response_id, "start": int(time.time())})
        self.game_state = "game-started-first-players-move"

        self.first_player_response_id += 1
        self.first_player_to_message_queue.append(
            {"name": "chat-message", "id": self.first_player_response_id, "start": int(time.time()),
             "payload": {"message": get_value_from_config("Greetings"), "owner": "qwe", "date": int(time.time())}})
        return
    ...
    def make_player_shoot(self, user_num, x, y):
        ...
        if user_num == 1:
            self.first_player_response_id += 1
            self.first_player_to_message_queue.append(
                {"name": first_user_action, "id": self.first_player_response_id, "start": int(time.time()),
                 "payload": {"register-self-shoot": {"y": y, "x": x, "state": result}}})
            self.second_player_response_id += 1
            self.second_player_to_message_queue.append(
                {"name": second_user_action, "id": self.second_player_response_id, "start": int(time.time()),
                 "payload": {"register-rival-shoot": {"y": y, "x": x, "state": result}}})
        else:
            self.first_player_response_id += 1
            self.first_player_to_message_queue.append(
                {"name": first_user_action, "id": self.first_player_response_id, "start": int(time.time()),
...
def waiting_for_event(user_id, json_data):
    ...
    events = game.get_unreceived_events_for_first_player()
    if events.__len__() == 0 and game.game_state == "second-player-move" and random.randint(0,
                                                                                            3) == 0 and game.ai_allowed:
